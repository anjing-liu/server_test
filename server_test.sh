...
create_shortcut() {
    ...
    if [ -f "$SCRIPT_PATH" ]; then
        if [ ! -f /usr/local/bin/sn ]; then
            ...
            [ $? -eq 0 ] && echo -e "${GREEN}✓ 快捷命令已创建：输入 'sn' 即可运行本脚本${PLAIN}"
        fi
...
detect_os() {
    if [ -f /etc/os-release ]; then
        ...
        if [[ "$ID" == "ubuntu" ]] || [[ "$ID" == "debian" ]]; then
            ...
            if [[ "$VERSION_ID" =~ ^7 ]]; then PKG_MANAGER="yum"; else PKG_MANAGER="dnf"; fi
        else
            PKG_MANAGER="unknown"
        fi
        ...
        PKG_MANAGER="unknown"; OS_ID="unknown"
    fi
}
detect_os
...
install_debian_deps() {
    ...
    if [ ${#missing_pkgs[@]} -gt 0 ]; then
        ...
        DEBIAN_FRONTEND=noninteractive apt install -y -qq "${missing_pkgs[@]}"
...
case $PKG_MANAGER in
    ...
    yum) install_centos7_deps ;;
    dnf) install_centos8_deps ;;
...
wget -qO /usr/local/bin/nexttrace "$NEXTTRACE_URL"
if [ -f /usr/local/bin/nexttrace ]; then
    chmod +x /usr/local/bin/nexttrace
    echo -e "${GREEN}[完成] nexttrace 安装成功${PLAIN}"
else
    curl -sSL https://raw.githubusercontent.com/sjlleo/nexttrace/main/install.sh | bash
...
lsmod | grep -q tcp_bbr && echo -e "${GREEN}BBR 模块已加载${PLAIN}" || echo -e "${RED}BBR 模块未加载${PLAIN}"
...
if command -v lscpu &>/dev/null; then
    ...
    [ -z "$CPU_FREQ_RAW" ] && CPU_FREQ_RAW=$(lscpu 2>/dev/null | grep "CPU动态频率" | awk -F':' '{print $2}' | xargs | cut -d'.' -f1)
    [ -z "$CPU_FREQ_RAW" ] && CPU_FREQ_RAW=$(lscpu 2>/dev/null | grep "CPU max MHz" | awk -F':' '{print $2}' | xargs | cut -d'.' -f1)
fi
if [ -z "$CPU_FREQ_RAW" ]; then
    CPU_FREQ_RAW=$(cat /proc/cpuinfo 2>/dev/null | grep "cpu MHz" | head -1 | awk -F':' '{print $2}' | xargs | cut -d'.' -f1)
fi
if [ -z "$CPU_FREQ_RAW" ] && [ -f /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq ]; then
    CPU_FREQ_RAW=$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq 2>/dev/null)
    [ -n "$CPU_FREQ_RAW" ] && CPU_FREQ_RAW=$((CPU_FREQ_RAW / 1000))
fi

if [ -n "$CPU_FREQ_RAW" ] && [ "$CPU_FREQ_RAW" -gt 0 ]; then
    if [ "$CPU_FREQ_RAW" -ge 1000 ]; then
        CPU_FREQ=$(echo "scale=1; $CPU_FREQ_RAW/1000" | bc)
        CPU_FREQ_UNIT="GHz"
    else
        CPU_FREQ="$CPU_FREQ_RAW"
        CPU_FREQ_UNIT="MHz"
    fi
else
    CPU_FREQ="未知"
...
[ -z "$CPU_USAGE" ] && CPU_USAGE=$(top -bn1 | grep "%Cpu" | awk '{print $2}')
...
for i in {1..3}; do
    ...
    rm -f /tmp/test_io /tmp/dd_output
    sync
...
if [ -n "$CPU_FREQ_UNIT" ]; then
    echo "CPU频率： ${CPU_FREQ} ${CPU_FREQ_UNIT}"
else
    echo "CPU频率： ${CPU_FREQ}"
fi
