...
# 检测系统（支持 Ubuntu/Debian/CentOS/Rocky/AlmaLinux）
detect_os() {
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   服务器一键测试脚本 v7.1${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${GREEN}检测到系统: $OS_ID, 包管理器: $PKG_MANAGER${PLAIN}\n"

# 安装依赖（智能跳过，支持 Ubuntu/Debian/CentOS/Rocky/AlmaLinux）
...
if [ "$PKG_MANAGER" = "apt" ]; then
    ...
    if ! dpkg -l | grep -q "^ii  build-essential"; then
        echo -e "${BLUE}[安装] build-essential ...${PLAIN}"
    ...
    if ! rpm -q epel-release &>/dev/null; then
        echo -e "${BLUE}[安装] epel-release ...${PLAIN}"
        yum install -y epel-release
    fi
    for pkg in gcc gcc-c++ make; do
...
if [ "$CUR_HOST" != "$EXPECT_HOST" ]; then
    ...
    hostnamectl set-hostname "$EXPECT_HOST" 2>/dev/null || true
    echo "$EXPECT_HOST" > /etc/hostname
    if grep -q "127.0.1.1" /etc/hosts; then
        sed -i "s/127.0.1.1.*/127.0.1.1 $EXPECT_HOST/g" /etc/hosts
    else
        echo "127.0.1.1 $EXPECT_HOST" >> /etc/hosts
    fi
...
if [ "$CURRENT_ALGO" = "bbr" ]; then
    ...
    echo -e "${YELLOW}BBR 可能未生效，当前算法: $CURRENT_ALGO${PLAIN}\n"
fi
...
RX_GB=$(echo "scale=2; $RX_BYTES/1073741824" | bc 2>/dev/null || echo "0")
TX_GB=$(echo "scale=2; $TX_BYTES/1073741824" | bc 2>/dev/null || echo "0")
[ -z "$RX_GB" ] && RX_GB=0
[ -z "$TX_GB" ] && TX_GB=0
...
for i in 1 2 3; do
    ...
    dd_output=$(dd if=/dev/urandom of=/tmp/test_io bs=1M count=64 oflag=direct 2>&1)
...
if [ ${#IO_SPEEDS[@]} -eq 3 ] && [ -n "${IO_SPEEDS[0]}" ] && [ -n "${IO_SPEEDS[1]}" ] && [ -n "${IO_SPEEDS[2]}" ]; then
...
if [ -n "$AVG_SPEED" ] && [ "$(echo "$AVG_SPEED < 100" | bc 2>/dev/null)" = "1" ]; then
    LEVEL="一般"
elif [ -n "$AVG_SPEED" ] && [ "$(echo "$AVG_SPEED < 200" | bc 2>/dev/null)" = "1" ]; then
    LEVEL="中等"
elif [ -n "$AVG_SPEED" ] && [ "$(echo "$AVG_SPEED < 500" | bc 2>/dev/null)" = "1" ]; then
    LEVEL="良好"
else
    LEVEL="优秀"
fi
...
bash <(curl -Ls https://IP.Check.Place) 2>/dev/null || echo -e "${RED}IP 风险检查失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[3/9] 执行三网回程线路测试...${PLAIN}"
curl -sSf https://raw.githubusercontent.com/zhanghanyun/backtrace/main/install.sh | sh 2>/dev/null || echo -e "${RED}回程线路测试失败${PLAIN}"
curl -s https://raw.githubusercontent.com/anjing-liu/mtr_trace/main/mtr_trace.sh | bash 2>/dev/null || echo -e "${RED}MTR 追踪失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[4/9] 执行三网+教育网 IPv4 单线程测速...${PLAIN}"
echo "2" | bash <(curl -sL https://raw.githubusercontent.com/i-abc/Speedtest/main/speedtest.sh) 2>/dev/null || echo -e "${RED}测速失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[5/9] 执行流媒体解锁测试...${PLAIN}"
printf "66\n" | bash <(curl -Ls https://check.unlock.media) 2>/dev/null || echo -e "${RED}流媒体测试失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"
...
echo -e "${BLUE}[6/9] 执行全国五网ISP路由回程测试...${PLAIN}"
{
    ...
    sleep 1
    echo "8"
} | nexttrace --fast-trace 2>/dev/null || echo -e "${RED}路由追踪失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[7/9] 执行三网回程路由测试...${PLAIN}"
printf "y\n" | bash <(curl -Ls https://Net.Check.Place) -R 2>/dev/null || echo -e "${RED}路由测试失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[8/9] 执行 bench 性能测试...${PLAIN}"
wget -qO- bench.sh | bash 2>/dev/null || echo -e "${RED}bench 测试失败${PLAIN}"
echo -e "${GREEN}完成${PLAIN}\n"

echo -e "${BLUE}========================================${PLAIN}"
echo -e "${BLUE}[9/9] 执行超售测试...${PLAIN}"
wget --no-check-certificate -O memoryCheck.sh https://raw.githubusercontent.com/uselibrary/memoryCheck/main/memoryCheck.sh 2>/dev/null
if [ -f memoryCheck.sh ]; then
    chmod +x memoryCheck.sh
    ./memoryCheck.sh
    rm -f memoryCheck.sh
else
    echo -e "${RED}超售测试下载失败${PLAIN}"
fi
echo -e "${GREEN}完成${PLAIN}\n"
